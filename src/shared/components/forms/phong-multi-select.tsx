...
interface PhongMultiSelectProps {
    value?: number[] | null
...
/**
 * Multi-select component for phòng (phòng ban có cấp bậc là "Phòng")
 * Chỉ hiển thị các phòng ban có cap_do = "Phòng"
 * Can be used as custom component in GenericFormView
 */
export function PhongMultiSelect({
    value,
    onChange,
    placeholder = "Chọn phòng áp dụng...",
    disabled = false,
}: PhongMultiSelectProps) {
    ...
    // Filter chỉ các phòng có cấp bậc là "Phòng" (case-insensitive)
    const phongList = React.useMemo(() => {
        if (!phongBanList) return []
        return phongBanList.filter(pb => {
            const capDo = pb.cap_do?.toLowerCase().trim()
            return capDo === "phòng" || capDo === "phong"
        })
    }, [phongBanList])
    
    const selectedIds = (value as number[] | null) || []
    const allSelected = selectedIds.length === (phongList?.length || 0) && (phongList?.length || 0) > 0
    ...
    const handleSelectAll = React.useCallback(() => {
        if (disabled || !phongList) return
        if (allSelected) {
            onChange([])
        } else {
            onChange(phongList.map(pb => pb.id!).filter(id => id !== undefined && id !== null))
        }
    }, [onChange, allSelected, phongList, disabled])
    ...
    const selectedOptions = React.useMemo(() => {
        if (!phongList) return []
        return phongList.filter(pb => selectedIds.includes(pb.id!))
    }, [phongList, selectedIds])
            ...
            <PopoverTrigger asChild>
                ...
                >
                    <div className="flex flex-wrap gap-1 flex-1">
                        ...
                        ) : selectedOptions.length === (phongList?.length || 0) ? (
                            <span className="text-sm">Tất cả phòng ({(phongList?.length || 0)})</span>
                        ) : (
                    ...
                    <div className="p-2">
                        {filteredOptions.length === 0 ? (
                            <div className="py-6 text-center text-sm text-muted-foreground">
                                {searchQuery ? "Không tìm thấy phòng nào" : "Không có phòng nào"}
                            </div>
                        ) : (
