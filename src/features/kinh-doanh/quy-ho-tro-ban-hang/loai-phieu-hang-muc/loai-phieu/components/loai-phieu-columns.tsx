...
function NameCell({ name, id }: { name: string; id: number }) {
    ...
    return (
        <button
            onClick={() => navigate(`${loaiPhieuConfig.routePath}/${id}`)}
    ...
    return (
        <div className="flex items-center gap-2 justify-end pr-4 min-w-[100px]">
            ...
            <div onClick={(e) => e.stopPropagation()} className="shrink-0">
                <DeleteLoaiPhieuButton id={id} name={name} iconOnly />
            </div>
        </div>
    )
}

export const loaiPhieuColumns = (): ColumnDef<LoaiPhieu>[] => [
    // ⚡ Professional: Use generic select column utility
    createSelectColumn<LoaiPhieu>(),
    {
        accessorKey: "ten_loai_phieu",
        header: ({ column }) => <SortableHeader column={column} title="Tên Loại Phiếu" />,
        cell: ({ row }) => {
            const loaiPhieu = row.original
            return (
                <NameCell 
                    name={loaiPhieu.ten_loai_phieu} 
                    id={loaiPhieu.id!} 
                />
            )
        },
