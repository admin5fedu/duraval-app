...
function NameCell({ name, id }: { name: string; id: number }) {
    ...
    return (
        <button
            onClick={() => navigate(`${nhomChuyenDeConfig.routePath}/${id}`)}
    ...
    return (
        <div className="flex items-center gap-2 justify-end pr-4 min-w-[100px]">
            ...
            <div onClick={(e) => e.stopPropagation()} className="shrink-0">
                <DeleteNhomChuyenDeButton id={id} name={name} iconOnly />
            </div>
        </div>
    )
}

export const nhomChuyenDeColumns = (): ColumnDef<NhomChuyenDe>[] => [
    createSelectColumn<NhomChuyenDe>(),
    {
        accessorKey: "ten_nhom",
        header: ({ column }) => <SortableHeader column={column} title="Tên Nhóm" />,
        cell: ({ row }) => {
            const nhomChuyenDe = row.original
            return (
                <NameCell 
