...
export function NguoiThanListView({ 
    ...
    const getColumnTitle = React.useCallback((columnId: string) => {
        const column = columnsWithEmployeeMap.find((col) => col.id === columnId || col.accessorKey === columnId)
    ...
    return (
        ...
        <GenericListView
            ...
            enableVirtualization={(nguoiThanList || []).length > 100}
            virtualRowHeight={60}
            exportOptions={{
                columns: columnsWithEmployeeMap,
                totalCount: nguoiThanList?.length || 0,
