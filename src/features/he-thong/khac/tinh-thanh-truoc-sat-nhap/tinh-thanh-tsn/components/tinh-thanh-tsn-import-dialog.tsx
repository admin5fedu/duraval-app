...
type UseBatchUpsertTinhThanhTSNReturn = ReturnType<typeof useBatchUpsertTinhThanhTSN>

interface TinhThanhTSNImportDialogProps {
    open: boolean
    onOpenChange: (open: boolean) => void
    mutation?: UseBatchUpsertTinhThanhTSNReturn
}
...
): Partial<TinhThanhTSN>[] {
    return rows.map((row) => {
        const mapped: Partial<TinhThanhTSN> = {}
...
export function TinhThanhTSNImportDialog({ open, onOpenChange, mutation }: TinhThanhTSNImportDialogProps) {
    const defaultMutation = useBatchUpsertTinhThanhTSN()
    ...
    const handleImport = async (rows: Partial<TinhThanhTSN>[]): Promise<{
        success: boolean
        inserted: number
        updated: number
        failed: number
