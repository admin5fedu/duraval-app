/**
 * Tabs Component cho Tỉnh thành sau sát nhập
...
export function TinhThanhSauSatNhapTabs() {
  const location = useLocation()
  const navigate = useNavigate()

  // Xác định tab active dựa trên pathname
  const pathname = location.pathname
  const isPhuongXaSNN = pathname.startsWith("/he-thong/phuong-xa-snn")
  
  let activeTab = "tinh-thanh-ssn"
  if (isPhuongXaSNN) {
    activeTab = "phuong-xa-snn"
  }

  const handleTabChange = (value: string) => {
    if (value === "tinh-thanh-ssn") {
      navigate("/he-thong/tinh-thanh-ssn")
    } else if (value === "phuong-xa-snn") {
      navigate("/he-thong/phuong-xa-snn")
    }
  }

  const tabs = [
    { value: "tinh-thanh-ssn", label: "Tỉnh thành SSN", icon: Map },
    { value: "phuong-xa-snn", label: "Phường xã SNN", icon: Building2 },
  ]
