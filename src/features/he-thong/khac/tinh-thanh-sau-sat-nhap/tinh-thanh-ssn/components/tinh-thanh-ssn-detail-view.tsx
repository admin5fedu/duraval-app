...
interface TinhThanhSSNDetailViewProps {
  id: number
  initialData?: any
  onEdit?: () => void
  onBack?: () => void
...
export function TinhThanhSSNDetailView({ id, initialData, onEdit, onBack }: TinhThanhSSNDetailViewProps) {
  ...
  if (viewState.isError) {
    return (
      <DetailErrorState
        title="Không tìm thấy tỉnh thành SSN"
        message="Tỉnh thành SSN với ID này không tồn tại hoặc đã bị xóa."
        onBack={onBack}
        backUrl={onBack ? undefined : tinhThanhSSNConfig.routePath}
      />
    )
  }
  ...
  const handleEdit = () => {
    if (onEdit) {
      ...
      navigate(`${tinhThanhSSNConfig.routePath}/${id}/sua?returnTo=detail`)
    }
  }
