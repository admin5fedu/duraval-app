...
function NameCell({ name, id }: { name: string; id: number }) {
    ...
    return (
        <button
            onClick={() => navigate(`${tinhThanhSSNConfig.routePath}/${id}`)}
    ...
    return (
        <div className="flex items-center gap-2 justify-end pr-4 min-w-[100px]">
            ...
            <div onClick={(e) => e.stopPropagation()} className="shrink-0">
                <DeleteTinhThanhSSNButton id={id} name={name} iconOnly />
            </div>
        </div>
    )
}

export const tinhThanhSSNColumns = (): ColumnDef<TinhThanhSSN>[] => [
    createSelectColumn<TinhThanhSSN>(),
    {
