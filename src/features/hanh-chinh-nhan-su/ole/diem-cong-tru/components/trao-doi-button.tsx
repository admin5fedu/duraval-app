...
export function TraoDoiButton({ diemCongTru, onSuccess, variant = "default" }: TraoDoiButtonProps) {
  ...
  const traoDoiList: TraoDoiItem[] = React.useMemo(() => {
    if (!diemCongTru.trao_doi) return []
    
    try {
      let list: TraoDoiItem[] = []
      
      // Nếu là array, map và convert
      if (Array.isArray(diemCongTru.trao_doi)) {
        list = diemCongTru.trao_doi.map((item: any) => ({
      ...
      else if (typeof diemCongTru.trao_doi === 'object') {
        const item = diemCongTru.trao_doi as any
        list = [{
          noi_dung: item.noi_dung || "",
          nguoi_trao_doi_id: item.nguoi_trao_doi_id || item.nguoi_xac_nhan_id || null,
          nguoi_trao_doi: item.nguoi_trao_doi || item.nguoi_xac_nhan || "",
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Thêm trao đổi liên quan đến điểm cộng trừ này.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <ScrollArea className="h-48 w-full rounded-md border p-4">
                <TraoDoiHistory traoDoi={diemCongTru.trao_doi} variant="list" emptyMessage="Chưa có trao đổi" />
              </ScrollArea>
            </div>
