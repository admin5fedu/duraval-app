...
import { useGiaiDoanKhachBuon, useBatchDeleteGiaiDoanKhachBuon, useDeleteGiaiDoanKhachBuon } from "../hooks"
...
import { GiaiDoanKhachBuonImportDialog } from "./giai-doan-khach-buon-import-dialog"
import {
    AlertDialog,
    AlertDialogAction,
    AlertDialogCancel,
...
export function GiaiDoanKhachBuonListView({ 
    ...
    const deleteMutation = useDeleteGiaiDoanKhachBuon()
    const batchImportMutation = useBatchUpsertGiaiDoanKhachBuon()
    const module = giaiDoanKhachBuonConfig.moduleName
    const [importDialogOpen, setImportDialogOpen] = React.useState(false)
    const [deleteDialogOpen, setDeleteDialogOpen] = React.useState(false)
    const [rowToDelete, setRowToDelete] = React.useState<GiaiDoanKhachBuon | null>(null)
    ...
    return (
        <>
            <GenericListView
                ...
                batchDeleteConfig={{
                    itemName: "giai đoạn khách buôn",
                    moduleName: giaiDoanKhachBuonConfig.moduleTitle,
                    isLoading: batchDeleteMutation.isPending,
                    getItemLabel: (item: GiaiDoanKhachBuon) => item.ten_giai_doan || String(item.id),
                }}
                onImport={() => setImportDialogOpen(true)}
                isImporting={batchImportMutation.isPending}
                onAdd={handleAddNewClick}
                addHref={`${giaiDoanKhachBuonConfig.routePath}/moi`}
                onBack={() => {
                    navigate(giaiDoanKhachBuonConfig.parentPath)
                }}
                ...
                enableVirtualization={(giaiDoanKhachBuonList?.length || 0) > 100}
                virtualRowHeight={60}
                exportOptions={{
                    columns: columns,
                    totalCount: giaiDoanKhachBuonList?.length || 0,
                    moduleName: giaiDoanKhachBuonConfig.moduleTitle,
                    getColumnTitle,
                    getCellValue,
                }}
            ...
            <AlertDialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            Bạn có chắc chắn muốn xóa giai đoạn khách buôn <strong>{rowToDelete?.ten_giai_doan || rowToDelete?.id || "này"}</strong>? Hành động này không thể hoàn tác.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
