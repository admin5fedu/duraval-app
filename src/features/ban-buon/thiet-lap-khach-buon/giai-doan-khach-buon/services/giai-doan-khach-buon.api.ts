...
export class GiaiDoanKhachBuonAPI {
    ...
    static async create(input: CreateGiaiDoanKhachBuonInput): Promise<GiaiDoanKhachBuon> {
        // ma_giai_doan and tt are required
        if (!input.ma_giai_doan || !input.ma_giai_doan.trim()) {
            throw new Error("Mã giai đoạn là bắt buộc")
        }
        if (!input.tt || input.tt < 1) {
            throw new Error("Thứ tự là bắt buộc và phải lớn hơn 0")
        }
        
        // Sanitize input: convert empty strings to null for optional fields
        const sanitizedInput = {
            ...input,
            ma_giai_doan: input.ma_giai_doan.trim(),
    ...
    static async update(id: number, input: UpdateGiaiDoanKhachBuonInput): Promise<GiaiDoanKhachBuon> {
        ...
        if (input.ma_giai_doan !== undefined) {
            sanitizedInput.ma_giai_doan = input.ma_giai_doan && input.ma_giai_doan.trim() !== "" ? input.ma_giai_doan : undefined
        }
